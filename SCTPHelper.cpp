...
void initSockets()
{
    for (auto &pair : nodes)
    {
        ...
        int recv_buf_size = 1024 * 1024; // 1 MB buffer size
        if (setsockopt(node.listeningSocket, SOL_SOCKET, SO_RCVBUF, &recv_buf_size, sizeof(recv_buf_size)) < 0)
        {
            std::cerr << "Error setting receive buffer size for node " << node.UID << std::endl;
            close(node.listeningSocket);
            continue;
        }
    ...
    for (auto &pair : nodes)
    {
        ...
        for (unsigned int neighborUID : node.neighbors)
        {
            ...
            while (connect(sock, (struct sockaddr *)&address, sizeof(address)) < 0)
            {
                ...
                {
                    std::cerr << "Node " << node.UID << " attempting to connect to neighbor " << neighborUID << std::endl;
                    break;
                }
                else
                {
                    std::cerr << "Error connecting node " << node.UID << " to neighbor " << neighborUID << std::endl;
                    close(sock);
                    break;
                }
            }
